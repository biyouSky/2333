...
{
	MByteToWChar()( winString, _strContentCache );
	UIPostMessage( this, WM_UPDATE, 1, 0 );
}
...
void UISelectList::Draw()
{
	...
	for( UINT i=0; i<_list.size(); ++i )
	{
		...
		if( IsRun() )
		{
			int width = GetWidth()( rc )+i*20;
			int dx = width/_maxFrame;
			rc.left += dx*( _maxFrame-_frameCount );
		}

		UI2DFont font( _z, 16 );
		font( _list[i].c_str(), rc, UI2DFont::FONT_LEFT );
	}
}
...
void UIComboBox::Draw()
{
	...
	rc.left = rc.right-10;
	rc.right = rc.left +8;
	rc.top += GetHeight()( rc )/2-4;
	rc.bottom = rc.top+8;
	UI2DImage( "GUIResource.dll", IDB_DOWN_ARROW, _z )( rc );
}
...
void UIScrollBar::OnUpdate( UINT flag, void* data )
{
	if( flag==1 )
	{
		_pageScale = _pageScaleCache;
		CalcBarArea();
	}
	else if( flag==2 )
	{
		_posScale = _posScaleCache;
		if( _posScale>1-_pageScale )
			_posScale = 1-_pageScale;
		CalcBarArea();
	}
...
UIGrid::GridCellInfor::GridCellInfor( std::string str )
{
	_strContent = str;
	_wordPos = DT_LEFT;
...
void UIGrid::GridCellInfor::CreateControl( UIContainer *pUIContainer, int firstRowPos, int firstColumnPos, int id )
{
...
bool UIGrid::GridCellInfor::DeleteControl()
{
	// 检查控件是否存在
...
void UIGrid::GridCellInfor::MoveControl( RECT& rc )
{
	if( _pCtrl==NULL ) return;
...
void UIGrid::GridCellInfor::DrawControl()
{
	if( _pCtrl==NULL ) return;
...
void UIGrid::DrawSelectCELL()
{
	...
	GridCellInfor* pSelCell = &_cellArray[_selRowBegin][_selColumnBegin];
...
void UIGrid::DrawSelectCELLS( COLORREF& selectColor )
{
	...
	GridCellInfor* pCellBegin = &_cellArray[beginRow][beginColumn];
	GridCellInfor* pCellEnd = &_cellArray[endRow][endColumn];
...
void UIGrid::DrawCells()
{
	...
	for( UINT row=_beginDrawRow; row<_rowNum; ++row )
	{
		...
		for( UINT column=_beginDrawColumn; column<_columnNum; ++column )
		{
			...
			GridCellInfor& cell = _cellArray[row][column];
			//
			UI2DClipRect uiClip;
			rc = _unfixGridArea;
			OffsetRect( &rc, x_, y_ );
			...
			if( cell._pCtrl==NULL )
			{
				...
				OffsetRect( &rc, x_, y_ );
				rc.left += 2;
				font( cell._strContent, rc, UI2DFont::FONT_LEFT );
			}
	...
	if( _isFirstRowFix )
	{
		...
		{
			GridCellInfor& cell = _cellArray[0][column];
			if( cell._pCtrl==NULL )
			{
	...
	if( _isFirstColumnFix )
	{
		...
		{
			GridCellInfor& cell = _cellArray[row][0];
			if( cell._pCtrl==NULL )
			{
	...
	// 绘制fix区域第一个cell
	{
		...
		if( _isFirstRowFix && _isFirstColumnFix )
		{
			GridCellInfor& cell = _cellArray[0][0];
			if( cell._pCtrl==NULL )
			{
...
void UIGrid::CalcScrollBar()
{
	...
	{
		// 计算滚动条信息
		if( _isXScrollShow==false )
			_xScroll.SetPosScale( 0 );
		_isXScrollShow = true;
		//
		_xScroll.SetPageScale( (double)( GetWidth()( _gridArea )-1 )/_widthSum );
	...
	else
	{
		...
		_isYScrollShow = true;
		//
		_yScroll.SetPageScale( (double)( GetHeight()( _gridArea )-1 )/_heightSum );
...
void UIGrid::CalcDrawBeginRowColumn( UINT& beginRow, UINT& beginColumn )
{
	...
	for( UINT r=0; r<_rowNum; ++r )
	{
		...
		{
			GridCellInfor& cell = _cellArray[r][c];
			if( cell._pCtrl!=NULL )
...
void UIGrid::CalcCellsPos()
{
	...
	{
		GridCellInfor& cell = _cellArray[0][colum];
	...
	for( UINT row=1; row<_rowNum; ++row )
	{
		for( UINT colum=1; colum<_columnNum; ++colum )
		{
			GridCellInfor& cell = _cellArray[row][colum];
...
void UIGrid::SetNoSel()
{
	...
	{
		GridCellInfor& activeCell = _cellArray[_selRowBegin][_selColumnBegin];
...
void UIGrid::SetCellString( UINT row, UINT column, string str, bool refresh )
{
	Cache1 *pCache = new Cache1;
	pCache->_row = row;
	pCache->_column = column;
	pCache->_str = str;
	pCache->_flag = refresh?1:0;
...
void UIGrid::GetCellString( UINT row, UINT column, std::string& str )
{
	...
	{
		if( _cellArray[row][column]._controlType==GridCellInfor::EDIT )
		{
			UIEdit& edit = *(UIEdit*)( _cellArray[row][column]._pCtrl );
			edit.GetString( str );
		}
		else if( _cellArray[row][column]._controlType==GridCellInfor::CHECKBUTTON )
		{
			UICheckButton& but = *(UICheckButton*)( _cellArray[row][column]._pCtrl );
			but.GetString( str );
		}
		else if( _cellArray[row][column]._controlType==GridCellInfor::COMBOBOX )
		{
...
void UIGrid::AddRow( VECTOR_STRING& row )
{
...
void UIGrid::AddRows( vector<VECTOR_STRING >& rowList )
{
	Cache4* pCache = new Cache4;
...
bool UIGrid::GetCellCheckState( UINT row, UINT column, bool& checkState )
{
	...
	if( cell._pCtrl==NULL || cell._controlType!=GridCellInfor::CHECKBUTTON ) 
		return false;
...
void UIGrid::AddCellComboBoxString( UINT row, UINT column, std::string str )
{
	Cache1 *pCache = new Cache1;
	pCache->_row = row;
	pCache->_column = column;
	pCache->_str = str;
	UIPostMessage( this, WM_UPDATE, 13, (LPARAM)pCache );
}

void UIGrid::ClearCellComboBoxList( UINT row, UINT column )
{
	Cache1 *pCache = new Cache1;
	pCache->_row = row;
	pCache->_column = column;
...
void UIGrid::OnChar( TCHAR nChar )
{
	...
	if( pSelCell->_controlType == GridCellInfor::EDIT )
	{
		// 创建edit控件
		pSelCell->CreateControl( &_uiContainer, _firstRowPos, _firstColumnPos );
...
void UIGrid::OnUpdate( UINT flag, void* data )
{
	...
	{
		if( !OnSetCellString( data ) ) return;
	}
	else if( flag==5 )
	{
		OnAddRows( data );
	...
	{
		OnAddCellComboBoxString( data );
	}
	else if( flag==14 )
	{
		OnClearCellComboBoxList( data );
	}
	else if( flag==15 )
	{
...
void UIGrid::OnSetRowColumn( void* data )
{
	...
	{
		int dx = _rowNum-_rowHeightList.size();
		while( dx-->0 )	
	...
	{
		int dx = _columnNum-_columnWidthList.size();
		while( dx-->0 )	
	...
	{
		_cellArray.push_back( vector<GridCellInfor>() );
		for( UINT colum=0; colum<_columnNum; ++colum ) 
			_cellArray[row].push_back( GridCellInfor("") );
	}
	// 
	CalcCellsPos();
}
...
bool UIGrid::OnSetCellString( void* data )
{
	Cache1* pCache = (Cache1*)data;
	...
	if( row<_rowNum && column<_columnNum )
	{
		if( _cellArray[row][column]._pCtrl==NULL )
		{
			_cellArray[row][column]._strContent = pCache->_str;
		}
		else 
		{	// 若cell中控件存在 则设置控件的string
			...
			{
				UIEdit& edit = *(UIEdit*)( _cellArray[row][column]._pCtrl );
				edit.SetString( pCache->_str );
			}
			else if( _cellArray[row][column]._controlType==GridCellInfor::CHECKBUTTON )
			{
				UICheckButton& but = *(UICheckButton*)( _cellArray[row][column]._pCtrl );
				but.SetString( pCache->_str );
			}
		}
	}
...
void UIGrid::OnAddRows( void* data )
{
	...
	for( UINT i=0; i<pCache->_rowList.size(); ++i )
	{
		...
		_cellArray.push_back( vector<GridCellInfor>() );
		for( UINT colum=0; colum<_columnNum; ++colum )
			_cellArray[_rowNum].push_back( GridCellInfor( rowCache[colum] ) );
		// 计算该行每个cell的位置
		for( UINT colum=0; colum<_columnNum; ++colum )
		{
			GridCellInfor& cell = _cellArray[_rowNum][colum];
...
void UIGrid::OnSetCellCheckButton( void* data )
{
	...
	cell._controlType = GridCellInfor::CHECKBUTTON;
	cell._strContent = pCache->_str;
	cell.CreateControl(  &_uiContainer, _firstRowPos, _firstColumnPos, id );
...
void UIGrid::OnSetCellCheckState( void* data )
{
	...
	{
		GridCellInfor& cell = _cellArray[pCache->_row][pCache->_column];
...
void UIGrid::OnSetCellComboBox( void* data )
{
	...
	if( row<_rowNum && column<_columnNum )
	{
		...
		cell._controlType = GridCellInfor::COMBOBOX;
		cell.CreateControl(  &_uiContainer, _firstRowPos, _firstColumnPos, id );
	}
}

void UIGrid::OnAddCellComboBoxString( void* data )
{
	Cache1* pCache = (Cache1*)data;
	UINT& row = pCache->_row;
	UINT& column = pCache->_column;
	...
	if( row<_rowNum && column<_columnNum )
	{
		...
		if( cell._pCtrl!=NULL )
		{
			...
			pControl->AddString( pCache->_str );
		}
	}
}
...
void UIGrid::OnSetCellButton( void* data )
{
	...
	if( row<_rowNum && column<_columnNum )
	{
		...
		cell._controlType = GridCellInfor::BUTTON;
		cell.CreateControl(  &_uiContainer, _firstRowPos, _firstColumnPos, id );
	}
}
...
bool UIChart::CurveInfor::CalcCoordRange( double& xMin, double&xMax, double& yMin, double& yMax )		
{
	// 相关检查
...
// 判断点p0是否靠近线( p1,p2 )
bool UIChart::CurveInfor::JudgePointNearLine( POINT& p0, POINT& p1, POINT& p2 )						
{
	// 处理直线垂直的情况
	if( p1.x == p2.x )
	{
...
bool UIChart::CurveInfor::operator==( std::string strName ) 
{  
...
void UIChart::DrawCurve( CurveInfor& curve, int yFlag )
{
	LONG& x_ = _abusolutePoint.x;
	LONG& y_ = _abusolutePoint.y;
...
void UIChart::CalcCurveDrawRange( CurveInfor& curve, int mode )
{
	// 是否存在点
...
bool UIChart::IsPointNearCurveLine( POINT& point, CurveInfor& curve, int yFlag )
{
	if( !curve._isLine )	
		return false;
...
bool UIChart::IsPointNearCurvePoint( POINT& point, CurveInfor& curve, int yFlag )
{
	VECTOR_POINT::iterator beginPointItor, endPointItor;
	beginPointItor = curve._pointList.begin()+curve._beginPointNO;
	endPointItor = curve._pointList.begin()+curve._endPointNO;
...
void UIChart::CalcXYCoordRange( bool isRefresh )
{
	UIPostMessage( this, WM_UPDATE, 0, isRefresh?1:0 );
}
...
void UIChart::AddCurve1( std::string strName, double xValue, double yValue, bool isRefresh )
{
	Cache2 *pCache = new Cache2;
	pCache->_x = xValue;
	pCache->_y = yValue;
	pCache->_curveName = strName;
	pCache->_isRefresh = isRefresh;
	pCache->_yFlag = 1;
...
void UIChart::AddCurve2( std::string strName, double xValue, double yValue, bool isRefresh )
{
	Cache2 *pCache = new Cache2;
	pCache->_x = xValue;
	pCache->_y = yValue;
...
bool UIChart::OnLButtonDown( POINT pt )
{
	...
	//
	for_each( _curveList1.begin(), _curveList1.end(), []( CurveInfor& curve ) { curve._isSelect = false; } );
	for_each( _curveList2.begin(), _curveList2.end(), []( CurveInfor& curve ) { curve._isSelect = false; } );
...
void UIChart::OnUpdate( UINT flag, void* data )
{
	...
	if( flag==0 )
	{
		isRefresh = OnCalcXYCoordRange( data );
	}
	else if( flag==1 )
	{
		OnAddCurve1( data );
...
bool UIChart::OnAddCurve2( void* data )
{
	Cache2* pCache = (Cache2*)data;
	double& x = pCache->_x;
	double& y = pCache->_y;
	bool isRefresh =  pCache->_isRefresh;
