...
public:
	...
	bool Create( UINT id, UIContainer* pUIContainer, RECT& relativeRect=NULLRECT, int layoutFlag=UILayoutCalc::NO_ZOOM, bool isShow=true, bool isOnHeap=false )
	{
		_id = id;
		return ( (TBase*)this )->CreateDirect( pUIContainer, relativeRect, layoutFlag, isShow, isOnHeap );
	}

	bool Create( UINT id, UIWinBase* pParent, RECT& relativeRect=NULLRECT, int layoutFlag=UILayoutCalc::NO_ZOOM, bool isShow=true, bool isOnHeap=false )
	{
		_id = id;
