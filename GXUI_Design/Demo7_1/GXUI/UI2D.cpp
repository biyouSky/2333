...
UI2DLine::UI2DLine( int startX, int startY, int endX, int endY, float z )
{
	_startX = (float)startX;
	_startY = (float)startY;
	_endX = (float)endX;
	_endY = (float)endY;
	_z = z;
}

UI2DLine::UI2DLine( POINT startPoint, POINT endPoint, float z )
{
	_startX = (float)startPoint.x;
	_startY = (float)startPoint.y;
...
void UI2DLine::operator()( D3DCOLOR rgbS, D3DCOLOR rgbE, UCHAR alphy )
{
	...
	UI2DVertex quad[2];
	quad[0] = UI2DVertex( _startX, _startY, _z, colorS );			// v1
	...
	points[0] = D3DXVECTOR2( _startX, _startY );
...
UI2DRect::UI2DRect( int startX, int startY, ULONG width, ULONG height, float z )
{
	_startX = (float)startX;
	_startY = (float)startY;
	_width = (float)width;
...
{
	_startX = (float)rect.left;
	_startY = (float)rect.top;
...
void UI2DRect::operator()( D3DCOLOR rgb )
{
	...
	UI2DVertex quad[5];
	quad[0] = UI2DVertex( _startX, _startY, _z, rgb );							// v1
	quad[1] = UI2DVertex( _startX+_width-1, _startY, _z, rgb );					// v2
	quad[2] = UI2DVertex( _startX+_width-1, _startY+_height-1, _z, rgb );		// v4
	quad[3] = UI2DVertex( _startX, _startY+_height-1, _z, rgb );				// v3
	quad[4] = UI2DVertex( _startX, _startY, _z, rgb );							// v1
...
void UI2DRect::operator()( D3DCOLOR rgbUp, D3DCOLOR rgbDown, UCHAR alphy )
{
	...
	*/
	UI2DVertex quad[4];
	quad[0] = UI2DVertex( _startX+_width, _startY, _z, colorUp );			// v2
	quad[1] = UI2DVertex( _startX+_width, _startY+_height, _z, colorDown );	// v4
	quad[2] = UI2DVertex( _startX, _startY, _z, colorUp );					// v1
	quad[3] = UI2DVertex( _startX, _startY+_height, _z, colorDown );		// v3
...
void UI2DImage::operator()( RECT& srcRect, ULONG dstStartX, ULONG dstStartY, UCHAR alphy )
{
	RECT dstRect = GenRect()( GenPoint()( dstStartX, dstStartY ), GenSize()( GetWidth()( srcRect ), GetHeight()( srcRect ) ) );
...
void DrawFrameHelp::operator()( ULONG dstStartX, ULONG dstStartY, ULONG width, ULONG height, UCHAR alphy )
{
	...
	RECT rc5d = GenRect()( GenPoint()( dstStartX+_leftBarWidth, dstStartY+height-_bottomBarHeight ), GenSize()( width-_leftBarWidth-_rightBarWidth, _bottomBarHeight ) );
	...
	RECT rc7d = GenRect()( GenPoint()( dstStartX, dstStartY+_topBarHeight ), GenSize()( _leftBarWidth, height-_topBarHeight-_bottomBarHeight ) );
	...
	RECT rc8d = GenRect()( GenPoint()( dstStartX+width-_rightBarWidth, dstStartY+_topBarHeight ), GenSize()( _rightBarWidth, height-_topBarHeight-_bottomBarHeight ) );
	...
	RECT rc9d = GenRect()( GenPoint()( dstStartX+_leftBarWidth, dstStartY+_topBarHeight ), GenSize()( width-_leftBarWidth-_rightBarWidth, height-_topBarHeight-_bottomBarHeight ) );
