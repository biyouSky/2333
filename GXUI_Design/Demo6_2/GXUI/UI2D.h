...
struct UI2DLine
{
	UI2DLine( int startX, int startY, int endX, int endY, float z=0.5f );
	UI2DLine( POINT startPoint, POINT endPoint, float z=0.5f );
	~UI2DLine();

	void operator()( D3DCOLOR rgbS, D3DCOLOR rgbE, UCHAR alphy );			// 颜色渐变线 只支持实线方式
	void operator()( D3DCOLOR rgb, UCHAR alphy=255 );						// 单色直线

	float _startX, _startY, _endX, _endY, _z;
};
...
struct UI2DRect
{
	UI2DRect( int startX, int startY, ULONG width, ULONG height, float z=0.5f );
	...
	float _startX, _startY, _width, _height, _z;
};
...
struct UI2DImage
{
	...
	void operator()( RECT& srcRect, ULONG dstStartX, ULONG dstStartY, UCHAR alphy=255 );
...
public:
	...
	void operator()( ULONG dstStartX, ULONG dstStartY, ULONG width, ULONG height, UCHAR alphy=255 );
