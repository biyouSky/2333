...
{	
	...
	// 删除播放完毕的动画
	_animateList.remove_if( std::not1( std::mem_fun<bool, UIAnimateBase>( &UIAnimateBase::IsRun ) ) );
...
void UIAnimateEffect::PlayAnimate( int mode )
{
	_animateMode = mode;
...
bool UIAnimateEffect::IsRun() 
{ 
	return _frameCount<_maxFrame ? true:false; 
}
...
void UIAnimateEffect::DrawHitDrumAnimate( int centerX, int centerY, float _z )
{
	...
	image.operator()( centerX, centerY, 1+0.8f*_frameCount/HIT_DRUM_FRAME, 255-250*_frameCount/HIT_DRUM_FRAME );
}
