...
public:
	...
	static void Destroy();
	static void Assign( UIMessageLoopManage* pPara );
...
void UIRefresh( bool immediately=false );
bool UIMessageFilter1( UINT message );		// 进入UI消息循环过滤
bool UIMessageFilter2( UINT message );		// 逆向传递消息透射过滤 
...
public:
	...
	bool CreatePopup( UIWinBase* pParent, RECT& relativeRect=_NULLRECT_, int layoutFlag=UILayoutCalc::NO_ZOOM, bool isShow=true, bool isOnHeap=false );
	// 销毁窗口
	bool DestroyDirect();
	...
	// 设置 直接无保护
	void MoveWindowDirect( RECT& relativeRect );
	void ShowWindowDirect( bool flag );
...
protected:
	...
	bool _isUpTransmissionMsg;					// 透射子窗口传递的消息
...
public:
	...
	// 设置
	void SetFocusOnChild( UIWinBase* pNew );
	void SetUnFocusAll();
...
private:	
	UIWinBase* GetMinZChild( const POINT& pt );
