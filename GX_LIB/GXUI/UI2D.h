...
struct UI2DImage
{
	...
	UI2DImage( UINT id, float z=0.5f, D3DCOLOR colorKey=D3DFMT_UNKNOWN );							// 从当前进程中获取图片数据 并绘制
	~UI2DImage();
	...
	void Init( UINT id, float z=0.5f, D3DCOLOR colorKey=D3DFMT_UNKNOWN );
...
public:
	...
	void Init( std::string imagePath, int topBarHeight, int bottomBarHeight, int leftBarWidth, int rightBarWidth, 
				   float z=0.5f, D3DCOLOR colorKey=D3DFMT_UNKNOWN );
	void Init( std::string resDLLPath, UINT id, int topBarHeight, int leftBarWidth, int rightBarWidth, int leftRightBarWidth, 
				   float z=0.5f, D3DCOLOR colorKey=D3DFMT_UNKNOWN );
