...
UI2DImage::UI2DImage( UINT id, float z, D3DCOLOR colorKey )
{
	_pITex = NULL;
	_pISprite = NULL;
	_isDraw = false;
...
void UI2DImage::Init( UINT id, float z, D3DCOLOR colorKey )
{
	_isDraw = false;
...
void UI2DImage::operator()( RECT& dstRect, float scale, UCHAR alphy )
{
	...
	this->operator()( srcRect, CalcZoomRect()( dstRect, scale ), alphy );
}
...
RECT UI2DImage::GetSrcRect()
{
	return GenRect()( GenPoint()( 0, 0 ), GenSize()( _srcInfo.Width, _srcInfo.Height ) );
}

DrawFrameHelp::DrawFrameHelp()
{
	_isDraw = false;
}

DrawFrameHelp::DrawFrameHelp( std::string imagePath, int topBarHeight, int bottomBarHeight, int leftBarWidth, int rightBarWidth, 
							  float z, D3DCOLOR colorKey )
{
	_isDraw = false;
	Init( imagePath, topBarHeight, bottomBarHeight, leftBarWidth, rightBarWidth, z, colorKey );
}
...
{
	_isDraw = false;
	Init( resDLLPath, id, topBarHeight, bottomBarHeight, leftBarWidth, rightBarWidth, z, colorKey );
}

void DrawFrameHelp::Init( std::string imagePath, int topBarHeight, int bottomBarHeight, int leftBarWidth, int rightBarWidth, 
						  float z, D3DCOLOR colorKey )
{
	_image.Init( imagePath, z, colorKey );
	...
	_isDraw = _image._isDraw;
}

void DrawFrameHelp::Init( std::string resDLLPath, UINT id, int topBarHeight, int bottomBarHeight, int leftBarWidth, int rightBarWidth, 
						  float z, D3DCOLOR colorKey )
{
	_image.Init( resDLLPath, id, z, colorKey );
