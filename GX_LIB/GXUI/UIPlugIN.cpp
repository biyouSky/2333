...
bool PlugINOBJ::Load( std::string path )
{
	...
	PFunc_PlugIN_Info PlugINInfo = ( PFunc_PlugIN_Info )GetProcAddress( _handle, "PlugINInfo" );
	CheckNULL( PlugINInfo )
	...
	PFunc_PlugIN_Menu PlugINMenu = ( PFunc_PlugIN_Menu )GetProcAddress( _handle, "PlugINMenu" );
	CheckNULL( PlugINMenu )
	...
	// 获得menu
	int num = 0;
	string* strList = PlugINMenu( num );
	for( int i=0; i<num; ++i )
		_menu.push_back( strList[i] );
	_menuSelIndex = -1;
...
void UIMainBar::SetTipMsg( std::string str )
{
...
{
	if( _isHover==flag ) return;

	_isHover = flag;

	if( !IsRunAnimate() || _animateMode!=HIT_DRUM )
		PlayAnimate( flag ? UIAnimateEffect::SET_HOT : UIAnimateEffect::SET_COOL );
}
...
void UIStartScreen::DrawBKImage()
{
	if( _bgImageID!=0 )
	{
		UI2DImage image( _bgImageID );
		RECT srcRC = image.GetSrcRect();
		RECT dstRC = srcRC;

		int dx = GetWidth()( _clientRC )-GetWidth()( srcRC );
		int dy = GetHeight()( _clientRC )-GetHeight()( srcRC );

		if( _bgImagePos==0 )
		{
			dstRC = _clientRC;
		}
		/*else if( _bgImagePos==1 )
		{
		}*/
		else if( _bgImagePos==2 )
		{
			OffsetRect( &dstRC, dx, 0 );
		}
		else if( _bgImagePos==3 )
		{
			OffsetRect( &dstRC, 0, dy );
		}
		else if( _bgImagePos==4 )
		{
			OffsetRect( &dstRC, dx, dy );
		}
		else if( _bgImagePos==5 )
		{
			OffsetRect( &dstRC, 0, dy/2 );
		}
		else if( _bgImagePos==6 )
		{
			OffsetRect( &dstRC, dx, dy/2 );
		}
		else if( _bgImagePos==7 )
		{
			OffsetRect( &dstRC, dx/2, 0 );
		}
		else if( _bgImagePos==8 )
		{
			OffsetRect( &dstRC, dx/2, dy );
		}

		image( dstRC, 1.f, _bgImageAlphy );
	}
}
...
void UIStartScreen::SetMainBarTipMsg( std::string msg )
{
	_mainBar.SetTipMsg( msg );
}

void UIStartScreen::SetBKImage( int id, int pos, int alphy )
{
	_bgImageID = id;
	_bgImagePos = pos;
	_bgImageAlphy = alphy;
