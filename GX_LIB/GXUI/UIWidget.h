...
/*
	添加规则：   2013.12.6
	1、控件进行设置一般不会主动刷新；（起因：控件内进行大数据量修改时导致的大量刷新而导致UI异常缓慢）
	2、添加bool _isCalcArea，每个控件的Draw()函数内必须首先判定_isCalcArea，若为真则调用CalcArea()，
	   这样可以把区域计算推迟到绘制前；（起因：控件进行大量数据设置时导致的大量中间计算是没有意义的，只需要在绘制前进行即可）
*/
...
protected:
	...
	bool OnMouseMove( POINT pt ) { return false; }
	...
	void CalcArea() {}
	void JudgeIsCalcArea()
	{
		if( _isCalcArea )
		{
			static_cast<T*>( this )->CalcArea();
			_isCalcArea = false;
		}
	}

	bool _isCalcArea;						// 是否调用CalcArea()，用于绘制前进行判定
};
...
public:
	...
	void SetPageScaleDirect( double s );
	void SetPosScaleDirect( double s );
...
class UIGrid : public UIControl< UIGrid >, public UIContainerHelp< UIGrid >
{
	...
	struct GridCellInfo
	{
		...
		D3DCOLOR _color;
		DWORD _wordPos;						// string位置 左中右

		VECTOR_STRING _cacheStrList;		// 控件用缓存信息
...
public:
	...
	// 行高列宽
	void SetRowHeight( UINT row, int height=20 );
	void SetColumnWidth( UINT column, int width=60 );
	...
	// 内容设置
	// 设置读取cell的字符
	void SetCellString( UINT row, UINT column, std::string str );
	void GetCellString( UINT row, UINT column, std::string& str );
	void SetCellColor( UINT row, UINT column, D3DCOLOR color );
	void SetCellStringAutoWidth( UINT row, UINT column, std::string str );
	// 添加一行或多行
	void SetRow( UINT row, VECTOR_STRING& strList );	
	void AddRow( VECTOR_STRING& row, DWORD wordPos=4 );						
	void AddRows( std::vector<VECTOR_STRING >& rowList, DWORD wordPos=4 );
	...
	//void SetCheckButtonMutex( std::vector<std::pair<UINT, UINT> >& mutexList );
	// 设置cell中comboBox控件
	void SetCellComboBox( int id, UINT row, UINT column, VECTOR_STRING& strList, bool isHold=true );
	UIComboBox* GetCellComboBox( UINT row, UINT column );
	// 设置cell fix信息
	void SetCellFix( UINT row, UINT column );
	void SetAllCellsFix();

	// 获得所有选中信息
	bool GetSelectCell( UINT& row, UINT& column );
	bool GetSelectCells( UINT& beginRow, UINT& beginColumn, UINT& endRow, UINT& endColumn );
	void GetSelectRows( VECTOR_UINT& selectRowList );
...
private:
	...
	void OnSetRowFix();
	void OnSetColumnFix();
	bool OnSetCellString( void* data, bool isAutoWidth=false );
	...
	bool OnSetCellColor( void* data );
	void OnSetCellFix( void* data );
	void OnSetAllCellsFix();
	void OnSetRow( void* data );
	//void OnSetCheckButtonMutex( void* data );
	...
	void SetCellStringDirect( UINT row, UINT column, std::string& str );
	// 粘贴时字符流数据解析
	...
	VECTOR_UINT _selRowList, _selColumnList;				// 选中的行或列
	...
	struct Cache6
	{
		...
		int _id;
		bool _isHold;
		VECTOR_STRING _strList;
	};
	struct Cache7
	{
		std::vector<std::pair<UINT, UINT> > _pairList;
	};
};
...
public:
	...
	void AddCurve1( std::string strName, double xValue, double yValue );
	...
	void AddCurve2( std::string strName, double xValue, double yValue );
...
private:
	...
	RECT _arrowRC1;
	RECT _arrowRC2;
	bool _isHoverArrowRC1;
	bool _isHoverArrowRC2;
